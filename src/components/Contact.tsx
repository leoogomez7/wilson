  ...
  return (
    <section id="contacto" className="py-5 md:py-8 px-6 md:px-20 bg-brand-black text-white">
      <div className="w-full">
        ...
        {sent ? (
          <div className="text-center py-12 border border-white/20">
...
function Field({
  ...
  return (
    <div className="space-y-2">
      <label className="text-[10px] uppercase tracking-[0.3em] opacity-40">{label}</label>
      <input
        name={name}
        type={type}
        required={required}
        placeholder={placeholder}
        className="w-full bg-transparent border-b border-white/20 py-4 focus:outline-none focus:border-white transition-colors placeholder:text-white/30"
      />
    </div>
  );
}
